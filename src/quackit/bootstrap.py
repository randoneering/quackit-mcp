...
def _create_postgres_storage(_database_path: Path) -> StorageBackend:
    ...
    if settings.database_url is None:
        raise ValueError(
            "QUACKIT_DATABASE_URL (or AGENT_MEMORY_DATABASE_URL) must be set for Postgres backend"
        )
...
def create_app_context(
    ...
    settings = load_settings()
    resolved_path = (
        settings.duckdb_path if database_path is None else Path(database_path)
    )
    factory = storage_factory or (
        _create_postgres_storage if settings.database_url else _create_storage
    )
    ...
    service = MemoryService(
        storage=storage, heartbeat_interval=settings.heartbeat_interval
    )
    return AppContext(

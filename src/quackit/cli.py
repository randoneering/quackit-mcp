...
def validate_transport_binding(
    ...
    if not allow_network and resolved_host not in _LOCALHOSTS:
        raise ValueError(
            "Refusing to bind MCP server to a non-localhost interface without "
            "--allow-network. Network transports expose memory tools without "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="quackit", description="Session-scoped agent memory MCP server"
    )
    parser.add_argument(
        "--database-path", type=Path, default=None, help="Path to DuckDB database file"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    start_parser = subparsers.add_parser(
        "start-session", help="Start a new memory session"
    )
    start_parser.add_argument(
        "--project-id", help="Optional project ID to associate session with"
    )
    ...
    activate_parser = subparsers.add_parser(
        "activate-session", help="Activate an existing session"
    )
    activate_parser.add_argument("session_id", help="Session ID to activate")

    save_parser = subparsers.add_parser(
        "save-memory", help="Save a memory in the active session"
    )
    save_parser.add_argument(
        "--session-id", required=True, help="Session ID to save memory to"
    )
    save_parser.add_argument(
        "--type",
        choices=[member.value for member in MemoryType],
        required=True,
        help="Memory type (context|summary|error_fix|note)",
    )
    ...
    save_parser.add_argument(
        ...
        help="Content format hint (note|code|markdown|skill)",
    )
    save_parser.add_argument(
        "--tag", action="append", default=[], help="Tag to attach (repeatable)"
    )
    save_parser.add_argument(
        "--metadata",
        type=json.loads,
        default=None,
        help='JSON dict of structured metadata (e.g. \'{"language":"python","framework":"pytest"}\')',
    )

    search_parser = subparsers.add_parser(
        "search-memory", help="Search memories in the active session"
    )
    search_parser.add_argument("query", help="Search query text")
    search_parser.add_argument(
        "--session-id", required=True, help="Session ID to search within"
    )
    search_parser.add_argument(
        "--type",
        choices=[member.value for member in MemoryType],
    ...
    search_parser.add_argument(
        "--content-type",
        choices=[member.value for member in ContentType],
        help="Filter by content type (note|code|markdown|skill)",
    )
    search_parser.add_argument(
        "--project-scope",
        action="store_true",
        default=False,
        help="Search across entire project",
    )

    update_parser = subparsers.add_parser(
        "update-memory", help="Update a memory's fields"
    )
    update_parser.add_argument("mem_id", help="Memory ID to update")
    update_parser.add_argument(
        "--session-id", required=True, help="Session ID owning the memory"
    )
    ...
    update_parser.add_argument(
        "--content-type",
        choices=[member.value for member in ContentType],
        help="New content type",
    )
    update_parser.add_argument(
        "--tag",
        action="append",
        default=[],
        help="Replace tags with this list (repeatable) — provide --tag to set tags, omit to leave unchanged",
    )
    update_parser.add_argument(
        "--metadata",
        type=json.loads,
        default=None,
        help='JSON dict of structured metadata (e.g. \'{"language":"python","framework":"pytest"}\') — provide to set, omit to leave unchanged',
    )

    get_parser = subparsers.add_parser(
        "get-memory", help="Get a specific memory by mem_id"
    )
    get_parser.add_argument("mem_id", help="Memory ID to retrieve")

    end_parser = subparsers.add_parser(
        "end-session", help="End the active session with a summary"
    )
    ...
    consolidate_parser = subparsers.add_parser(
        "consolidate-projects", help="Consolidate multiple projects into one"
    )
    consolidate_parser.add_argument(
        "--source-ids", nargs="+", required=True, help="Project IDs to consolidate from"
    )
    consolidate_parser.add_argument(
        "--target-id", required=True, help="Project ID to consolidate into"
    )

    subparsers.add_parser("list-projects", help="List all projects")

    list_sessions_parser = subparsers.add_parser(
        "list-sessions-by-project", help="List sessions for a project"
    )
    list_sessions_parser.add_argument(
        "project_id", help="Project ID to list sessions for"
    )
    ...
    save_skill_parser.add_argument(
        "--content", required=True, help="Skill instructions/content"
    )
    save_skill_parser.add_argument(
        "--tag", action="append", default=[], help="Tag to attach (repeatable)"
    )
    ...
    update_skill_parser = subparsers.add_parser(
        "update-skill", help="Update a skill's fields"
    )
    ...
    update_skill_parser.add_argument(
        ...
        action="append",
        default=None,
        help="Replace tags with this list (repeatable)",
    )
    ...
    serve_parser.add_argument(
        "--host", default=None, help="Host to bind (SSE/HTTP only)"
    )
    serve_parser.add_argument(
        "--port", type=int, default=None, help="Port to bind (SSE/HTTP only)"
    )
    serve_parser.add_argument(
        "--allow-network",
        action="store_true",
        help="Allow non-localhost binding for SSE/HTTP transports",
    )
    ...
    try:
        ...
        if args.command == "list-sessions":
            _dump(
                [
                    item.model_dump()
                    for item in service.list_recent_sessions(limit=args.limit)
                ]
            )
            return 0
        ...
        if args.command == "save-memory":
            ...
            content_type = (
                None if args.content_type is None else ContentType(args.content_type)
            )
            _dump(
                service.save_memory(
        ...
        if args.command == "search-memory":
            ...
            _dump(
                [
                    item.model_dump()
                    for item in service.search_memory(
                        query=args.query,
                        type=memory_type,
                        content_type=content_type,
        ...
        if args.command == "consolidate-projects":
            _dump(
                service.consolidate_projects(
                    source_ids=args.source_ids, target_id=args.target_id
                ).model_dump()
            )
            return 0
        ...
        if args.command == "list-sessions-by-project":
            _dump(
                [
                    item.model_dump()
                    for item in service.list_sessions_by_project(args.project_id)
                ]
            )
            return 0
        ...
        if args.command == "list-skills":
            _dump(
                [
                    item.model_dump()
                    for item in service.list_skills(query=args.query, tag=args.tag)
                ]
            )
            return 0
        ...
        if args.command == "serve":
            ...
            host = validate_transport_binding(
                args.transport, args.host, args.allow_network
            )
            auth_config = load_oauth_config_from_env()
            if host is not None:
                kwargs["host"] = host
            ...
            build_server(
                ...
                auth_config=auth_config if args.transport != "stdio" else None,
            ).run(transport=args.transport, **kwargs)
            return 0

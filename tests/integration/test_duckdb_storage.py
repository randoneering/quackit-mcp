...
from quackit.models import ContentType, MemoryCreate, MemoryType, MemoryUpdate, ProjectRecord, SessionStatus, SkillCreate
...
def test_storage_consolidate_projects_moves_sessions_and_memories(tmp_path: Path) -> None:
    storage = DuckDBStorage(tmp_path / "memory.duckdb")
...
def test_storage_update_memory_metadata_partial(tmp_path: Path) -> None:
    ...
    created = storage.save_memory(
        session_id=session.id,
        memory=MemoryCreate(type=MemoryType.NOTE, content="original", tags=["keep"], metadata={"old": "data"}),
    )
...
def test_storage_save_and_get_skill(tmp_path: Path) -> None:
    ...
    created = storage.save_skill(
        SkillCreate(name="test-skill", description="A test", content="print('hello')", tags=["python", "test"], source="/tmp/test.py"),
    )
...
def test_storage_list_skills(tmp_path: Path) -> None:
    ...
    s3 = storage.save_skill(SkillCreate(name="gamma", content="ccc", tags=["python", "cli"]))
